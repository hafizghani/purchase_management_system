...
def fillingquotation(request):
    ...
    try: 
        request_of_quotations = RequestOfQuotation.objects.get(request_of_quotation_id = re_of_quo_id)
        item_list = RequestOfQuotationItem.objects.filter(request_of_quotation_id = re_of_quo_id)
        context = {
                'title': 'Quotation Form',
                'quotation_id': 'QUO' + str(quo_id),
                'request_of_quotation_id': re_of_quo_id, 
                'staff_id' : staff.person_id,
                'vendor_id': request_of_quotations.vendor_id.vendor_id,
                'rows':item_list
            }
        return render(request,'Quotation/quotationform.html',context)

    except RequestOfQuotation.DoesNotExist:

        context = { 'error': 'The request of quotation id is invalid !',
                    'title': 'Quotation Form'
            }
        return render(request,'Quotation/quotationform.html',context)
...
def quotationconfirmation(request):
    ...
    request_of_quotation_id = request.POST['request_of_quotation_id']
    user_id  = request.user.id
    staff = Person.objects.get(user_id = user_id)
    ...
    context = {
            ...
            'request_of_quotation_id' : request_of_quotation_id,
            'quotation_id' : quo_id,
            'staff_id' : staff.person_id,
            'vendor_id' : vendor_id,
...
def quotationdetails(request):
    ...
    vendor_id = request.POST['vendor_id']
    description = request.POST['description']
    request_of_quotation = RequestOfQuotation.objects.get(request_of_quotation_id = request_of_quotation_id)
    ...
    quo_info = Quotation(quotation_id = quo_id, 
                            ...
                            request_of_quotation_id = request_of_quotation)
    quo_info.save()
...
def quotationhistorydetails(request):
    ...
    context = {
            ...
            'title': 'Quotation Details',
            'request_of_quotation_id' : quotation.request_of_quotation_id.request_of_quotation_id,
            'quotation_id' : pk,
